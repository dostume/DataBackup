...
import com.xayah.core.datastore.readStreamUpload
...
class PackagesBackupUtil @Inject constructor(
    ...
    private val volumeBackupUtil: VolumeBackupUtil,
) {
    companion object {
    ...
    /**
     * Backs up apk archives to cloud using volume splitting and, optionally,
     * streaming upload.
     */
    suspend fun backupApkToCloud(
        p: PackageEntity,
        ...
        volumeBackupUtil.compressAndUpload(
            ...
            command = command,
            dstDir = dstDir,
            remoteDstDir = remoteDstDir,
            baseName = dataType.type,
            suffix = ct.suffix,
            volumeSize = context.readVolumeSize().first(),
            ...
            if (result.isSuccess) {
                p.setDataBytes(dataType, sizeBytes)
                p.setDisplayBytes(dataType, uploaded)
            }
        }
    ...
    /**
     * Backs up package data (user/user_de/data/obb/media) to cloud using volume
     * splitting and, optionally, streaming upload.
     */
    suspend fun backupDataToCloud(
        ...
        log { "Backing up ${dataType.type} to cloud..." }
        ...
        val command = Tar.buildCompressCommand(
            exclusionList = exclusionList,
            h = if (context.readFollowSymlinks().first()) "-h" else "",
            srcDir = srcDir,
            src = packageName,
