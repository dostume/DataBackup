...
class MediumRestoreUtil @Inject constructor(
    ...
    suspend fun download(
        ...
        val ct = m.indexInfo.compressionType
        val src = mediaRepository.getArchiveDst(dstDir = srcDir, ct = ct)
        val baseName = DataType.MEDIA_MEDIA.type
        val suffix = ct.suffix

        t.updateInfo(state = OperationState.DOWNLOADING)

        val hasVolumes = runCatching {
            client.listFiles(srcDir).files.any { file ->
                parseVolumePartFileName(file.name)?.let { it.baseName == baseName && it.suffix == suffix } == true
            }
        }.getOrDefault(false)
