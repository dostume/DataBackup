...
internal class BackupServiceCloudImpl @Inject constructor() : AbstractBackupService() {
    ...
    override suspend fun backup(type: DataType, p: PackageEntity, r: PackageEntity?, t: TaskDetailPackageEntity, dstDir: String) {
        ...
        if (mContext.readVolumeSize().first() > 0) {
            if (type == DataType.PACKAGE_APK) {
                mPackagesBackupUtil.backupApkToCloud(p = p, t = t, dstDir = dstDir, client = mClient, remoteDstDir = remoteAppDir)
            } else {
                mPackagesBackupUtil.backupDataToCloud(p = p, t = t, dataType = type, dstDir = dstDir, client = mClient, remoteDstDir = remoteAppDir)
            }
        } else {
