...
import com.xayah.core.model.util.extensionWithoutVolume
import com.xayah.core.model.util.suffixOf
import com.xayah.core.model.util.volumePartIndex
        ...
        typedPathSet.forEach { typed ->
            runCatching {
                ...
                archives.forEach { archivePath ->
                    if (archivePath.extension.volumePartIndex() > 0) return@forEach
                    val archiveExtension = archivePath.extension.extensionWithoutVolume()
                    // For each archive
                    ...
                    runCatching {
                        when (archivePath.nameWithoutExtension) {
                            DataType.PACKAGE_APK.type -> {
                                ...
                                val type = CompressionType.suffixOf(archiveExtension)
                                if (type != null) {
                                    ...
                                    if (context.readReloadDumpApk().first() && archivePath.extension.volumePartIndex() < 0) {
                                    ...
                                    log { "Failed to parse compression type: $archiveExtension" }
                                }
                            }
